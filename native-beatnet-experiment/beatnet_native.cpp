...
extern "C" {
extern const unsigned char beatnet_weights_begin[];
extern const unsigned char beatnet_weights_end[];
}
...
struct Weights {
  const float* conv_w;
  const float* conv_b;
  const float* projection_w;
  const float* projection_b;
  const float* lstm0_w;
  const float* lstm0_r;
  const float* lstm0_b;
  const float* lstm1_w;
...
void LstmLayer(const float* input, const float* input_weights,
               const float* recurrent_weights, const float* biases,
               float* output) {
  ...
  for (int frame = 0; frame < kFrames; ++frame) {
    ...
    // ONNX gate order is input, output, forget, cell.
    for (int index = 0; index < kProjection; ++index) {
...
bool Predict(const float* features, float* activations) {
  ...
  LstmLayer(projected.data(), weights.lstm0_w, weights.lstm0_r,
            weights.lstm0_b, layer0.data());
  LstmLayer(layer0.data(), weights.lstm1_w, weights.lstm1_r,
            weights.lstm1_b, layer1.data());
  ...
  for (int frame = 0; frame < kFrames; ++frame) {
    ...
    for (int output = 0; output < kClasses; ++output) {
      ...
      for (int index = 0; index < kProjection; ++index) {
        // val_156 is [150,3], unlike the row-major projection matrix above.
        sum += layer1[frame * kProjection + index] *
               weights.output_w[index * kClasses + output];
      }
      logits[output] = sum;
    }
    ...
    // The ONNX Slice keeps [0, 2), i.e. the first two softmax classes.
...
std::size_t EmbeddedWeightBytes() {
  return static_cast<std::size_t>(beatnet_weights_end - beatnet_weights_begin);
}
